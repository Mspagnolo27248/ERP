...
export class ORMODBC {


  static async findAll<T extends Record<string, any>>(): Promise<any> {
    const tableName = this.getTableName();
    const db  = await  odbc.connect(CONNECTION_STRING)
    const rawTableRecords = await db.query(`SELECT * FROM ${tableName}`);
    await db.close();
  ...
  //Protected methods
  private static getModelToTableFieldMap() {
    const columns =
